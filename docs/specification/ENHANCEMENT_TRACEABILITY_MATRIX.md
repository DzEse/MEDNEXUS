...
| A17 | Geospatial enterprise entities | IMPLEMENTED/PARTIAL | Phase 12G canonically merges role-specific Plant/Supplier/Customer geography and adds DimWarehouse with SIMULATED_ENTERPRISE_FOOTPRINT; no shared active DimRegion is admitted | Validate actual Power BI map interactions and drill behavior |
...
| A27 | Command Center drillable data paths | IMPLEMENTED/PARTIAL | Phase 12G canonically implements Plant→Warehouse plus Plant/Shift/Employee and Department→JobRole→EmployeeAssignment paths; unsafe direct Line/Department/Region paths remain excluded | Validate actual Power BI interactions and drill-through behavior |
...
| A29 | Prototype-before-expansion validation | IMPLEMENTED/PARTIAL | Phase 12E.1 passes 62/62 prototype checks; Phase 12F reviews all 20 structures; Phase 12G applies only the approved 8 structures and passes the canonical 58-export semantic/data-quality gate | Complete local Phase 12G evidence and actual PBIX reconciliation before further expansion |
...
| B07 | Enterprise Operations Map | DATA-READY / BUILD DEFERRED | Phase 12G canonically provides role-specific Plant/Supplier/Customer/Warehouse geography and rejects ambiguous shared Region filtering | Build and validate map in Power BI Desktop; label SIMULATED ENTERPRISE FOOTPRINT |
| B08 | Map entity interaction | PARTIAL / CANONICAL CONTRACT VALIDATED | Phase 12G implements Plant→Warehouse and role-specific geography semantics while excluding a global active Region path | Validate actual map cross-filter/drill behavior in Power BI Desktop |
...
| B18 | High-value filter architecture | PARTIAL / CANONICAL CONTRACT VALIDATED | Phase 12G implements the approved Plant/Shift/Employee/Department/JobRole/Warehouse paths with no ambiguous canonical paths; supplier/customer geography remains role-specific | Test actual PBIX filter behavior before acceptance |
...
5. run the local Phase 12F evidence/storage benchmark — **completed**;
6. prototype Power BI filter-behavior contract simulation — **completed: 29/29 checks, 0 semantic issues**;
7. physically apply only the 8 approved structures in Phase 12G — **completed**;
8. regenerate the canonical semantic-model contract and reconciliation targets — **completed; local Phase 12G evidence next**;
9. build and reconcile the actual Power BI model;
