...
| 37 | Python architecture | IMPLEMENTED/PARTIAL | Modular `mednexus/` package now includes governance/observability, statistical quality, root-cause statistics and model explainability layers | Add dedicated feature-engineering/forecast/scenario-optimization modules only where later requirements justify them; notebooks remain exploratory |
...
| 42 | Testing & validation | IMPLEMENTED/PARTIAL | 40-test suite covering reproducibility, governance, KPI aggregation, FPY/SPC/loss gates, SQL reconciliation, ML temporal/model/threshold/calibration/explainability, root-cause CIs/FDR/effect tests/robust regression/VIF/tree and non-causal interpretation | Forecast comparator, MORI sensitivity, scenario/optimization/process gates and later analytics still require dedicated tests |
